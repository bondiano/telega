...
pub fn decode(json: Dynamic) -> Result(Message, dynamic.DecodeErrors) {
  let decode = update_decoder()
  use raw_message_update <- try(decode(json))

  Ok(raw_message_to_message(raw_message_update.message))
}
...
fn decode_raw_message(
  raw_message: Dynamic,
) -> Result(RawMessage, dynamic.DecodeErrors) {
  ...
  let decode_reply_to_message =
    dynamic.optional_field("reply_to_message", decode_raw_message)
  ...
  case
    decode_message_id(raw_message),
    decode_message_thread_id(raw_message),
    decode_from(raw_message),
    decode_sender_chat(raw_message),
    decode_sender_boost_count(raw_message),
    decode_date(raw_message),
    decode_chat(raw_message),
    decode_is_topic_message(raw_message),
    decode_is_automatic_forward(raw_message),
    decode_reply_to_message(raw_message),
    decode_via_bot(raw_message),
    decode_edit_date(raw_message),
    decode_has_protected_content(raw_message),
    decode_media_group_id(raw_message),
    decode_author_signature(raw_message),
    decode_text(raw_message),
    decode_entities(raw_message),
    decode_caption(raw_message),
    decode_caption_entities(raw_message),
    decode_has_media_spoiler(raw_message),
    decode_new_chat_members(raw_message),
    decode_left_chat_member(raw_message),
    decode_new_chat_title(raw_message),
    decode_delete_chat_photo(raw_message),
    decode_group_chat_created(raw_message),
    decode_supergroup_chat_created(raw_message),
    decode_channel_chat_created(raw_message),
    decode_migrate_to_chat_id(raw_message),
    decode_migrate_from_chat_id(raw_message),
    decode_connected_website(raw_message),
    decode_inline_keyboard_markup(raw_message)
  {
...
fn update_decoder() {
  dynamic.decode2(
    Update,
    dynamic.field("message", decode_raw_message),
